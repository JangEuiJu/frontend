...
export default function App ( props ) {
    ...
    const countJsx = (
        <>
            {/* 
                - 버튼을 클릭하면 상태변수 +1 증가 -> 화면갱신 
                - 클릭이벤트 등록, inline 방식으로 이벤트핸들러 등록          
            */}
    ...
    // B-3. JSX 생성
    // div 요소 준비, onClick 이벤트 등록, 화살표함수 이벤트핸들러 등록
    ...
    const condiJsx = (
        <>
            ...
            <div onClick={ ()=> { 
                ...
                setPageContent( pageContent ? null : '내용 세팅됨' )  
            } }>
                {/* null  자체는 랜더링 x */}
                pageContent = [ {pageContent} ]
                <br/>
                {/* null 조건식에 위치하면 무조건 false */}
                { pageContent ? "참일때 값" : "거짓일때 값" }
            </div>
        </>
    );
