/**
 * - 게시판 형태로 UI를 구성
 *  - 메모리제이션 사용 -> 최적화!!, 메모리 절약(상대적)
 *  - 자주 사용되는 데이터나 함수를 캐싱하여서 화면갱신시 매번 생성 x, 바로 사용
 *      - 데이터는 바로 엑세스 (통신, 전처리 등 과정 x)
 *      - 함수 -> 매번 생성 x, 바로 호출
 */
...
import {
  ...
  useMemo, // 데이터
  useCallback, // 함수, 함수호출은 매번 작동, 함수의 정의 1회, 이후는 캐싱
...
import {
    ...
    // TODO 게시판-7 mui card는 생략
    Card,           // 특정 장면을 위한 컨테이너
    CardActions,    // 카드상의 이벤트
    CardContent,    // 카드 내용
    CardMedia,      // 카드 내 멀티미디어 처리(사진, 음악, 동영상) 삽입
...
export default function App({ url, pid }) {
  ...
  const [product, setProduct] = useState(null); // 1개 상품에 대한 상세정보 JSX
  ...
  // TODO 게시판-6 mui 상태 변수, 이벤트 함수, 기타 필요 기능
  const [open, setOpen] = useState(false); // 팝업 오픈, 닫는 역할 담당
  const dCloseHandler = () => {
    // 팝업 닫기
    setOpen(false);
    // 1개 상품 정보 초기화
    ...
    <div className="App">
      <div className="App-header">
        ...
        <div style={{ margin: "2em" }}>
          ...
          <br />
          {/* TODO 게시판-2 mui 관련 JX내의 테이블 교체 */}
          <TableContainer component={Paper}>
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              <TableHead>
                {/* 컬럼 표현, 기존의 tr-th */}
                <TableRow>
                  <TableCell>상품명 (100g serving)</TableCell>
                  <TableCell align="right">가격</TableCell>{" "}
                  {/* 수치는 오른쪽 정렬 */}
                  <TableCell align="left">카테고리(g)</TableCell>
                  <TableCell align="left">설명(g)</TableCell>
                </TableRow>
              </TableHead>
          ...
          {/* onClose : 비어있는 공간을 클릭 */}
            ...
            >
                    ...
                    <DialogContent>
                        {/* product 내용은 card 컴포넌트로 재구성 교체 예정 */}
                        {product}
                    </DialogContent>
                    <DialogActions>
                        <Button onClick={dCloseHandler} autoFocus>
                            닫기
                        </Button>
                    </DialogActions>
            </Dialog>
