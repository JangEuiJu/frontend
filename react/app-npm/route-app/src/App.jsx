/**
 *  App.jsx : 전체 화면을 관장하는 컴포넌트, 상단:링크, 하단 내용
 */
...
function App() {
  ...
  return (
    <>
      <fieldset>
        {/* 하위 페이지, 특정 페이지로 이동하는 링크 제공, 지속적 노출 
            - URL 링크 등록시 => /붙여서 표현
            - a 태그 사용  X => 사용 시 화면 껌뻑 -> SPA가 아님, 데이터가 모두 리셋
            - 빈칸 1개 : &nbsp <= html
        */}
        <nav>
          <Link to="/">■ 홈페이지</Link>
      ...
      <fieldset>
        {/* - 라우트 되어야 하는 페이지들을 등록, 개별페이지 세팅
            - 페이지에 따라 변경
            - Routes 하위에 등록된 컴포넌트들은 이 영역에서만 페이지가 교체됨
            */}
        <Routes>
          {/* Route 등록 : 1개가 URL이 되고, 특정 컴포넌트 세팅
                - path='URL' => 해당 컴포넌트를 볼 수 있는 URL 세팅
                ex) /, /xxxx, /xxx/xxxx/.... <= 백엔드 구성하는 방식
                ex) /, xxxx, /xxxxx/xxxx <= 상위그룹, 하위그룹 형태로 구성
                - element => 해당 컴포넌트 요청시 보이는 화면(컴포넌트)
          */}
          ...
          <Route path="dashboard">
            {/* /dashboard <= index를 Route내부에 표기 
                - 특정 비지니스 로직을 가진 URL들의 대표 홈페이지가 된다.
            */}
            ...
            */}
            ...
            {/* /dashboard/데이터/detail, 데이터가 주소의 중간에 위치 */}
